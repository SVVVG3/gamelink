-- Migration: 006_create_groups_schema_fixed.sql
-- Description: Create groups and memberships tables for group functionality (FIXED)
-- Author: GameLink Development Team
-- Created: 2024-12-19

-- ========================================
-- UTILITY FUNCTIONS (Create missing functions)
-- ========================================

-- Create the missing trigger_set_timestamp function
...
CREATE INDEX IF NOT EXISTS idx_groups_primary_game ON groups(primary_game);
CREATE INDEX IF NOT EXISTS idx_groups_gaming_platform ON groups(gaming_platform);
...
CREATE INDEX IF NOT EXISTS idx_groups_description_search ON groups USING gin(to_tsvector('english', description));
...
CREATE INDEX IF NOT EXISTS idx_group_memberships_last_active ON group_memberships(last_active_at);
...
-- Drop existing group_memberships policies if they exist
DROP POLICY IF EXISTS "Users can view memberships of groups they belong to" ON group_memberships;
...
-- Drop existing group_invitations policies if they exist
...
DROP POLICY IF EXISTS "Users can respond to their invitations" ON group_invitations;
...
DROP TRIGGER IF EXISTS trigger_add_group_creator_as_admin ON groups;
DROP TRIGGER IF EXISTS trigger_update_group_stats_on_membership ON group_memberships;
DROP TRIGGER IF EXISTS trigger_update_groups_updated_at ON groups;
DROP TRIGGER IF EXISTS trigger_update_group_memberships_updated_at ON group_memberships;
DROP TRIGGER IF EXISTS trigger_update_group_invitations_updated_at ON group_invitations;
...
BEGIN
    ...
    IF sample_user_id IS NOT NULL THEN
        -- Only insert if groups don't already exist
        INSERT INTO groups (name, description, primary_game, gaming_platform, skill_level, created_by) 
        SELECT * FROM (VALUES
        ...
        ) AS new_groups(name, description, primary_game, gaming_platform, skill_level, created_by)
        WHERE NOT EXISTS (
            SELECT 1 FROM groups WHERE groups.name = new_groups.name AND groups.created_by = new_groups.created_by
        );
    END IF;
END $$;
...
-- Success message
SELECT 'Groups schema migration completed successfully!' as result; 